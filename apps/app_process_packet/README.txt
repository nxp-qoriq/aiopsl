...
3. 2. For LS1088a, copy the files ls1088a_sim_init_params.cfg and ls1088a_sys_test.cfg
      from the source tree at: aiopsl/misc/sim/.
      into the simulator folder at: dtsim_release/linux64/
3. 3. For LS2088a simulator, copy the files ls2088a_sim_init_params.cfg and ls2088a_sys_test.cfg
      from the source tree at: aiopsl/misc/sim/.
      into the simulator folder at: dtsim_release/linux64/
4. Copy the layout file (dpl.dtb) from aiopsl/misc/setup/ to the simulator folder.
5. Copy the configuration file (dpc.dtb) from aiopsl/misc/setup/ to the simulator folder.
6. Copy “eth_ipv4_udp.pcap” from the source tree at: aiopsl/misc/setup/ into the simulator folder

===========================================
Simulator (M147) Execution flow
===========================================
...
3. 3. For LS2088a:
      Run the simulator:
      ./ccssim2 -port 42333 -imodel "ls_sim_init_file=ls2088a_sim_init_params.cfg" -smodel "ls_sim_config_file=ls2088a_sys_test.cfg"
4. Launch mc_app using "mc_sim_dbg" launch configuration.
   Don't forget to update simulator server IP and port in debug configuration - 42333.
   Make sure the the target type in the target settings in CW is set to the right device (LS2085A, LS1088A, LS2088A)
 Note:
   You should check in the cccssim2 console that the correct target is simulated.
   One of these message should appear early in the ccssim2 output:
   *** Running the model as LS2085 ***
   *** Running the model as LS1088 ***
   *** Running the model as LS2088 ***
5. After MC reaches main(), run tio console:
   ./bin/tio_console -hub localhost:42975 -ser duart2_1 duart2_0
 Note:
   In case the libtio is not found, update the "LD_LIBRARY_PATH" variable to point to the simulator folder.
   setenv LD_LIBRARY_PATH {$LD_LIBRARY_PATH}:/home/user/LS_SIM_<version>/dtsim_release/linux64
6. Run mc_app.
7. Once the MC loads the AIOP code, you can attach to the "app_process_packet" (make sure to un-mark initialization files).
   In the MC console you should see this message:
   "Loaded AIOP version"
8. Run “tio capture”:
   ./bin/fm_tio_capture -hub localhost:42975 -ser w0_m1 -verbose_level 2
   Here you'll be able to capture sent and received packets.
   The note about "LD_LIBRARY_PATH" applies as for tio_console.
9. Run “tio inject”:
   ./bin/fm_tio_inject -hub localhost:42975 -ser w0_m1 -file eth_ipv4_udp.pcap -verbose_level 2
...
3. The user may use different tio port and update it inside ls2085a_sys_test.cfg or ls1088a_sys_test.cfg or ls2088a_sys_test.cfg
...
6. In case you want to dump all the default parameter values for a certain simulator device, run one of these commands:
	For ls2085:
	./bin/sim_LS2085 -n -s sim.printParams=true -s sim.printHiddenParams=true

	For ls1088:
	./bin/sim_LS1088 -n -s sim.printParams=true -s sim.printHiddenParams=true

	For ls2088:
	./bin/sim_LS2085 -n -s sim.ls2rev2=true -s sim.printParams=true -s sim.printHiddenParams=true
