...
static uint8_t ioread8(const volatile uint8_t *addr)
{
	uint8_t ret = *addr;
	core_memory_barrier();
	return ret;
...
static uint16_t ioread16be(const volatile uint16_t *addr)
{
...
static uint32_t ioread32be(const volatile uint32_t *addr)
{
...
static uint64_t ioread64be(const volatile uint64_t *addr)
{
...
static void iowrite8(uint8_t val, volatile uint8_t *addr)
{
	*addr = val;
	core_memory_barrier();
}
