...
int dpni_create(struct dpni *dpni, const struct dpni_cfg *cfg)
{
	...
	/* send command to mc*/
	return cmdif_open(&(dpni->cidesc), CMDIF_MOD_DPNI, DPNI_CMDSZ_CREATE,
			  (uint8_t *)&cmd_data, MC_OPT_CMD_CREATE);
}
...
int dpni_open(struct dpni *dpni, int dpni_id)
{
	...
	/* send command to mc*/
	return cmdif_open(&(dpni->cidesc), CMDIF_MOD_DPNI, MC_CMD_OPEN_SIZE,
			  (uint8_t *)&cmd_data, 0);
}
...
{
	/* send command to mc*/
	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_DESTROY,
		 ...
		 const struct dpni_pools_cfg *cfg)
{
	...
	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_SET_POOLS,
...
int dpni_attach(struct dpni *dpni, const struct dpni_attach_cfg *cfg)
{
	...
	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_ATTACH, DPNI_CMDSZ_ATTACH,
...
{
	/* send command to mc*/
	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_DETACH, DPNI_CMDSZ_DETACH,
			  CMDIF_PRI_LOW, NULL);
}
...
{
	/* send command to mc*/
	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_ENABLE, DPNI_CMDSZ_ENABLE,
			  CMDIF_PRI_LOW, NULL);
}
			      ...
			      struct dpni_buffer_layout *layout)
{
	struct mc_cmd_data cmd_data = { { 0 } };
	int err;
	err = cmdif_send(&(dpni->cidesc), DPNI_CMDID_GET_TX_BUFFER_LAYOUT,
			      ...
			      struct dpni_buffer_layout *layout)
{
	...
	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_SET_TX_BUFFER_LAYOUT,
			      ...
			      struct dpni_buffer_layout *layout)
{
	struct mc_cmd_data cmd_data = { { 0 } };
	int err;
	err = cmdif_send(&(dpni->cidesc), DPNI_CMDID_GET_TX_CONF_BUFFER_LAYOUT,
			      ...
			      struct dpni_buffer_layout *layout)
{
	...
	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_SET_TX_CONF_BUFFER_LAYOUT,
			  DPNI_CMDSZ_SET_TX_CONF_BUFFER_LAYOUT, CMDIF_PRI_LOW,
			  (uint8_t *)&cmd_data);
}

int dpni_get_l3_cksum_validation(struct dpni *dpni,
                                 int *en)
{
	struct mc_cmd_data cmd_data = { { 0 } };
	int err;
	err = cmdif_send(&(dpni->cidesc), DPNI_CMDID_GET_L3_CKSUM_VALIDATION,
	                 DPNI_CMDSZ_GET_L3_CKSUM_VALIDATION, CMDIF_PRI_LOW,
			 (uint8_t *)&cmd_data);
	if (!err) {
		/* retrieve response parameters */
		DPNI_RSP_GET_L3_CKSUM_VALIDATION(RSP_READ);
	}
	return err;
}

int dpni_set_l3_cksum_validation(struct dpni *dpni,
                                   int en)
{
	struct mc_cmd_data cmd_data = { { 0 } };

	DPNI_CMD_SET_L3_CKSUM_VALIDATION(CMD_PREP);

	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_SET_L3_CKSUM_VALIDATION,
		                 DPNI_CMDSZ_SET_L3_CKSUM_VALIDATION, CMDIF_PRI_LOW,
				 (uint8_t *)&cmd_data);
}

int dpni_get_l4_cksum_validation(struct dpni *dpni,
                                 int *en)
{
	struct mc_cmd_data cmd_data = { { 0 } };
	int err;
	err = cmdif_send(&(dpni->cidesc), DPNI_CMDID_GET_L4_CKSUM_VALIDATION,
	                 DPNI_CMDSZ_GET_L4_CKSUM_VALIDATION, CMDIF_PRI_LOW,
			 (uint8_t *)&cmd_data);
	if (!err) {
		/* retrieve response parameters */
		DPNI_RSP_GET_L4_CKSUM_VALIDATION(RSP_READ);
	}
	return err;
}

int dpni_set_l4_cksum_validation(struct dpni *dpni,
                                   int en)
{
	struct mc_cmd_data cmd_data = { { 0 } };

	DPNI_CMD_SET_L4_CKSUM_VALIDATION(CMD_PREP);

	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_SET_L4_CKSUM_VALIDATION,
		                 DPNI_CMDSZ_SET_L4_CKSUM_VALIDATION, CMDIF_PRI_LOW,
				 (uint8_t *)&cmd_data);
}
		     ...
		     uint32_t *fqid)
{
	...
	err = cmdif_send(&(dpni->cidesc), DPNI_CMDID_GET_RX_FLOW,
...
{
	struct mc_cmd_data cmd_data = { { 0 } };
	uint64_t vaddr = PTR_TO_UINT(cfg->extract_cfg);
	uint32_t size = sizeof(struct dpkg_profile_cfg);
			...
			uint8_t enable_state)
{
	...
	/* send command to mc*/
	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_SET_IRQ_ENABLE,
...
int dpni_set_irq_mask(struct dpni *dpni, uint8_t irq_index, uint32_t mask)
{
	...
	/* send command to mc*/
	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_SET_IRQ_MASK,
...
int dpni_clear_irq_status(struct dpni *dpni, uint8_t irq_index, uint32_t status)
{
	...
	/* send command to mc*/
	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_CLEAR_IRQ_STATUS,
...
int dpni_set_vlan_filters(struct dpni *dpni, int en)
{
	...
	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_SET_VLAN_FILTERS,
...
int dpni_set_vlan_insertion(struct dpni *dpni, int enable)
{
	...
	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_SET_VLAN_INSERTION,
...
int dpni_set_vlan_removal(struct dpni *dpni, int enable)
{
	...
	return cmdif_send(&(dpni->cidesc), DPNI_CMDID_SET_VLAN_REMOVAL,
