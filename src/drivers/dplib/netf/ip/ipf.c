...
int32_t ipf_generate_frag(struct ipf_context *ipf_ctx){
	...
	struct ipv4hdr *ipv4_hdr = (struct ipv4hdr *) pr->ip1_or_arp_offset /*! + Presentation location !*/;
	...
	//int first_frag = (ipf_ctx->flags & IPF_FIRST_FRAG); /* first fragment flag */
	int first_frag = ipf_ctx->first_frag; /* first fragment flag */
	int i;
	
	/* A LOT OF THINGS TO DO HERE */
	
	ip_header_len = ((ipv4_hdr->vsn_and_ihl & IPV4_HDR_IHL_MASK) >> IPV4_HDR_IHL_OFFSET) << 2;
	...
	/* !! THIS CODE DOES NOT PROTECT AGAINST MALICIOUS/MALFORMED OPTIONS !! (e.g. wrong length field in options */
	while(more_opt){
