...
int32_t tcp_gso_split_segment(struct tcp_gso_context *gso_ctx)
{
	...
	if (sr_status == FDMA_SPLIT_FRAME_UNABLE_TO_SPLIT_ERR) {
		...
		/* store remaining FD */
		sr_status = fdma_store_frame_data(gso_ctx->rem_frame_handle, spid, 
		...
		*((struct ldpaa_fd *)HWC_FD_ADDRESS) = gso_ctx->rem_fd;
		...
		/* TODO FDMA ERROR */
		sr_status = fdma_insert_segment_data(&insert_segment_data_params);
	...
	sr_status = l4_udp_tcp_cksum_calc(L4_UDP_TCP_CKSUM_CALC_OPTIONS_NONE); */
