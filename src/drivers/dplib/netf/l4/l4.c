		...
		uint16_t udp_src_port, uint16_t udp_dst_port)
{
	...
	if (~PARSER_IS_UDP_DEFAULT())
		return HM_NO_UDP_FOUND_ERROR;
	PARSER_CLEAR_RUNNING_SUM();
	if (flags & HM_UDP_MODIFY_MODE_UDPSRC)
		udp_ptr->src_port = udp_src_port;
	if (flags & HM_UDP_MODIFY_MODE_UDPDST)
		udp_ptr->dst_port = udp_dst_port;
	if (flags & HM_UDP_MODIFY_MODE_L4_CHECKSUM) {
		...
		int16_t tcp_ack_num_delta, uint16_t tcp_mss)
{
	...
	option_size = HM_TCP_NO_OPTION_SIZE;
	if (~PARSER_IS_TCP_DEFAULT())
		return HM_NO_TCP_FOUND_ERROR;
	PARSER_CLEAR_RUNNING_SUM();

	if (flags & HM_TCP_MODIFY_MODE_L4_CHECKSUM) {
		old_header = *(uint32_t *)tcp_ptr;
		if (flags & HM_TCP_MODIFY_MODE_TCPSRC)
			tcp_ptr->src_port = tcp_src_port;

		if (flags & HM_TCP_MODIFY_MODE_TCPDST)
			tcp_ptr->dst_port = tcp_dst_port;

		if (flags & (HM_TCP_MODIFY_MODE_TCPDST |
				HM_TCP_MODIFY_MODE_TCPSRC))
			cksum_update_uint32(&tcp_ptr->checksum,
				old_header,
				*(uint32_t *)tcp_ptr);

		if (flags & HM_TCP_MODIFY_MODE_SEQNUM) {
		...
		if (flags & HM_TCP_MODIFY_MODE_ACKNUM) {
		...
		if (flags & HM_TCP_MODIFY_MODE_MSS) {
			if (PARSER_IS_TCP_OPTIONS_DEFAULT())
				return HM_NO_TCP_MSS_FOUND_ERROR;
