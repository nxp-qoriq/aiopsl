...
int dpbp_create(struct dpbp *dpbp, const struct dpbp_cfg *cfg)
{
	...
	/* send command to mc*/
	return cmdif_open(&(dpbp->cidesc), CMDIF_MOD_DPBP, DPBP_CMDSZ_CREATE,
...
int dpbp_open(struct dpbp *dpbp, int dpbp_id)
{
	...
	/* send command to mc*/
	return cmdif_open(&(dpbp->cidesc), CMDIF_MOD_DPBP, MC_CMD_OPEN_SIZE,
			  (uint8_t *)&cmd_data, 0);
}
...
{
	/* send command to mc*/
	return cmdif_send(&(dpbp->cidesc), DPBP_CMDID_DESTROY,
...
{
	/* send command to mc*/
	return cmdif_send(&(dpbp->cidesc), DPBP_CMDID_ENABLE, DPBP_CMDSZ_ENABLE,
			  CMDIF_PRI_LOW, NULL);
}
		 ...
		 int irq_virt_id)
{
	...
	/* send command to mc*/
	return cmdif_send(&(dpbp->cidesc), DPBP_CMDID_SET_IRQ,
			...
			uint8_t enable_state)
{
	...
	/* send command to mc*/
	return cmdif_send(&(dpbp->cidesc), DPBP_CMDID_SET_IRQ_ENABLE,
...
int dpbp_set_irq_mask(struct dpbp *dpbp, uint8_t irq_index, uint32_t mask)
{
	...
	/* send command to mc*/
	return cmdif_send(&(dpbp->cidesc), DPBP_CMDID_SET_IRQ_MASK,
...
int dpbp_clear_irq_status(struct dpbp *dpbp, uint8_t irq_index, uint32_t status)
{
	...
	/* send command to mc*/
	return cmdif_send(&(dpbp->cidesc), DPBP_CMDID_CLEAR_IRQ_STATUS,
