...
int dpio_create(struct dpio *dpio, const struct dpio_cfg *cfg)
{
	...
	/* send command to mc*/
	return cmdif_open(&(dpio->cidesc), CMDIF_MOD_DPIO, DPIO_CMDSZ_CREATE,
...
int dpio_open(struct dpio *dpio, int dpio_id)
{
	...
	/* send command to mc*/
	return cmdif_open(&(dpio->cidesc), CMDIF_MOD_DPIO, MC_CMD_OPEN_SIZE,
			  (uint8_t *)&cmd_data, 0);
}
...
{
	/* send command to mc*/
	return cmdif_send(&(dpio->cidesc), DPIO_CMDID_DESTROY,
			  DPIO_CMDSZ_DESTROY, CMDIF_PRI_LOW, NULL);
}

int dpio_enable(struct dpio *dpio)
{
	/* send command to mc*/
	return cmdif_send(&(dpio->cidesc), DPIO_CMDID_ENABLE, DPIO_CMDSZ_ENABLE,
			  CMDIF_PRI_LOW, NULL);
}
		 ...
		 int irq_virt_id)
{
	...
	/* send command to mc*/
	return cmdif_send(&(dpio->cidesc), DPIO_CMDID_SET_IRQ,
			  DPIO_CMDSZ_SET_IRQ, CMDIF_PRI_LOW,
			  (uint8_t *)&cmd_data);
}
			...
			uint8_t enable_state)
{
	...
	/* send command to mc*/
	return cmdif_send(&(dpio->cidesc), DPIO_CMDID_SET_IRQ_ENABLE,
			  DPIO_CMDSZ_SET_IRQ_ENABLE, CMDIF_PRI_LOW,
			  (uint8_t *)&cmd_data);
}
...
int dpio_set_irq_mask(struct dpio *dpio, uint8_t irq_index, uint32_t mask)
{
	...
	/* send command to mc*/
	return cmdif_send(&(dpio->cidesc), DPIO_CMDID_SET_IRQ_MASK,
			  DPIO_CMDSZ_SET_IRQ_MASK, CMDIF_PRI_LOW,
			  (uint8_t *)&cmd_data);
}
...
int dpio_clear_irq_status(struct dpio *dpio, uint8_t irq_index, uint32_t status)
{
	...
	/* send command to mc*/
	return cmdif_send(&(dpio->cidesc), DPIO_CMDID_CLEAR_IRQ_STATUS,
