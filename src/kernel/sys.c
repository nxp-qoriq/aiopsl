...
__SHRAM t_sys_forced_object_desc sys_handle[NUM_OF_HANDLES];
...
{
	ASSERT_COND(module < NUM_OF_HANDLES && module >= 0);

	if(sys_handle[module].used == 1)
	{
		return sys_handle[module].h_module;
	}


	pr_err("sys_get_handle: Handle not exists");
	return NULL;
                   ...
                   int num_of_ids, ... )
{
	...
	if(sys_handle[module].used != 0)
	{
		pr_err("Handle already exists\n");
		return ENOMEM;
	}

	lock_spinlock((uint8_t *)&sys_handle[module].spinlock);
	sys_handle[module].h_module = h_module;
	sys_handle[module].module = module;
	sys_handle[module].used = 1;
	unlock_spinlock((uint8_t *)&sys_handle[module].spinlock);
	return E_OK;
...
int sys_remove_handle(enum fsl_os_module module, int num_of_ids, ... )
{
	...
	{
		lock_spinlock((uint8_t *)&sys_handle[module].spinlock);
		sys_handle[module].used = 0;
		unlock_spinlock((uint8_t *)&sys_handle[module].spinlock);
		return E_OK;
	}

	pr_err("sys_remove_handle: Handle not exists\n");
